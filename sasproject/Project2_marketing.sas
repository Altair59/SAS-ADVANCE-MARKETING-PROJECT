...
and where filter)*/
proc sql;
select count(target_b), sum(target_b) from scopva where p_1 > .075;
quit;
...
proc means data=pva_train n sum;
var target_b;
run;

proc means data=pva_test n sum;
var target_b;
run;
...
proc logistic data=pva_train des;
model target_b = MONTHS_SINCE_LAST_GIFT
INCOME_GROUP
RECENT_AVG_GIFT_AMT
PEP_STAR
...
proc sql;
select income_group, avg(p_1) as pred, avg(target_b) as act
from scored_test group by 1;
quit;
